...
def register_user(request:RegisterBuyerRequest,authentication_service:AuthenticationService=Depends(get_authentication_service)):
    return authentication_service.register_buyer(request)


@router.post("/login")
def login_user(request:LoginBuyerRequest,  authentication_service: AuthenticationService = Depends(get_authentication_service)):
